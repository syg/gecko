...
void
SpewTempOptimizationTypeInfoVector(const TempOptimizationTypeInfoVector *types, const char *indent = nullptr)
{
#ifdef DEBUG
...
void
SpewTempOptimizationAttemptsVector(const TempOptimizationAttemptsVector *attempts, const char *indent = nullptr)
{
#ifdef DEBUG
...
MaybeConstructorFromType(types::Type ty)
{
    if (!ty.isTypeObject())
        return nullptr;
    types::TypeObject *obj = ty.typeObject();
  ...
  public:
    ...
    void readType(const IonTrackedTypeWithAddendum &tracked) MOZ_OVERRIDE {
        ...
        if (ty.isPrimitive() || ty.isUnknown() || ty.isAnyObject()) {
            op_.readType(types::NonObjectTypeString(ty), nullptr, 0);
            return;
        }
        ...
        if (tracked.hasConstructor()) {
            JSFunction *constructor = tracked.constructor;
            PutEscapedString(buf, bufsize, constructor->displayAtom(), 0);

            ScriptSource *source;
            uint32_t lineno;
            if (constructor->hasScript()) {
                source = constructor->nonLazyScript()->maybeForwardedScriptSource();
                lineno = constructor->nonLazyScript()->lineno();
            } else {
                source = constructor->lazyScript()->maybeForwardedScriptSource();
                lineno = constructor->lazyScript()->lineno();
            }

            op_.readType(buf, source->introducerFilename(), lineno);
            return;
        }
